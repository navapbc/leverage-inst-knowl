...
| **VERIFIED (2026-07-28): the Catalog agent's MCP servers are `always_allow`, so the allowlist backstop (R14/R15) is INERT for it** — tool calls never pause, so the runner can't gate or deny them | Accepted for v1: a scheduled run writes exactly as today's interactive sync does (also `always_allow`, no human gate), and the effective bound is the agent's limited toolset (catalog register/list + Confluence reads) plus the skill's own DO-NOT-USE hold-back. To make R14/R15 actually apply, a write tool must be set to `permission_policy='ask'` (would also reintroduce interactive prompts) — deferred. The `auto_approve` allowlist is kept as documented intent and activates automatically if that policy ever changes. |
