...
A secondary goal is **operational BI and reporting**: the same Precompute Warehouse also serves deterministic dashboards and structured reporting outputs. This is an explicit, stated goal so that the deterministic data pipeline (DSs → Pipeline → PW → BI Dashboards) is justified alongside the primary AI-retrieval goal rather than being an unscoped add-on.
