...
* Skill and agent names carry **no** `lik-` prefix (e.g. skill dir `query-project-index`, agent file `knowledge-search.yaml`). The Claude workspace is single-tenant for this app, so no namespacing is needed. Applies to skill directories + their `name:` frontmatter and agent file stems.
    - **Renaming a catalog-writing skill requires a data migration.** A skill's `computed_by` literal (e.g. `sync-catalog-from-project-indexes`) is stored on every catalog row it writes **and** is part of the skill-row upsert key (`catalog_skill_key` on `(entry_type, subject, computed_by) WHERE row_provenance='skill'`). Renaming the skill without migrating existing rows' `computed_by` to the new value makes the next sync silently insert duplicates instead of updating in place, orphaning the old rows. When you rename such a skill, in the same change run a non-destructive `UPDATE catalog SET computed_by='<new>' WHERE computed_by='<old>' AND row_provenance='skill'` against prod (per the DB-schema-changes note above). This bit us once during the `lik-` prefix removal.
