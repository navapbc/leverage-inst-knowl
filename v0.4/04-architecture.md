...
- **Non-recomputable data** — captured human input that is no DS record and can't be derived from any DS: confirmation signals and human-created/verified artifacts. Durable: because it can't be regenerated from a DS, the only way to recover from a bad edit is to restore an earlier version, so it needs its own backup (see §4).
...
It is the one un-pointed-to artifact, so it lives at a **well-known address** agents know a priori. It starts as a **single Confluence page** and is treated as **just another DS artifact**, with one tightening: because it's the single entry point everyone hits first, **all writes go through a DL-creation skill's service account** — autonomously for rows it computes, under a verified human assertion for human-created rows; no one edits rows directly. Reads stay open. Consumers treat a **missing or malformed row as a cache miss** — fall back to skill routing or a bounded fan-out rather than erroring.
...
Most DL **by volume** is **recomputable** from DSs (indexes, aggregations, categorization, hints, Catalog, propagated ACLs). The exceptions are **non-recomputable data** — captured human input that is no DS record and can't be derived from any DS: **confirmation signals** and **human-created/verified artifacts**. Because these can't be regenerated from a DS, they're **durable**: they need their own backup/retention, and restoring an earlier version is the only way to recover from a bad edit.
