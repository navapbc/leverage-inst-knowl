...
### Discovery Layer (DL)
A **computed layer derived from DSs** — a *logical role, not a single store*. What makes something DL is **purpose, not location**: it exists to make DS knowledge faster to find and reuse, and never holds primary knowledge authored for its own sake. A DL output can physically sit in a DS (a summary in a Confluence page, tagged with a `discovery-layer` marker) and still be DL, because its role is discovery. It comprises:
- **Computed outputs** that speed retrieval: summaries, aggregations, indexes, categorization, prioritized pointers, retrieval hints, cross-reference/relationship maps, canonical-pointer/dedup, content-freshness/obsolescence signals, propagated access-control metadata.
- **A Catalog** mapping `type + subject → location`, so tools know where each output lives.
- **Non-recomputable data** — captured human input that is no DS record and can't be derived from any DS: confirmation signals and human-created/verified artifacts. Durable: the only way to recover from a bad edit is to restore an earlier version. A human-created/verified artifact lives in a DS, so that DS backs it up like any record; confirmation signals are the one part DL stores itself and must back up (see §4).
...
Most DL **by volume** is **recomputable** from DSs (indexes, aggregations, categorization, hints, Catalog, propagated ACLs) — cheap to rebuild, safe to discard. The exceptions are **non-recomputable data** — captured human input that is no DS record and can't be derived from any DS: **confirmation signals** and **human-created/verified artifacts**. These are **durable**: restoring an earlier version is the only way to recover from a bad edit.

**Backup follows location, not role.** A DL output that physically lives in a DS — including a durable human-created/verified one — is **backed up by that DS**, like any of its records; DL stands up no separate backup for it. The one non-recomputable output DL stores itself is the **confirmation signal**, so the confirmation store is the only thing DL must back up directly. (The Catalog is recomputable and needs no backup of its own, though in practice it often shares the confirmation store's database and is backed up alongside it.) Caveat: a DS-stored artifact's durability is only as good as its host DS's backup — if a target DS has weak retention or shallow version history, that is where recovery risk concentrates.
