...
- **Role:** `discovery-layer` — a **role marker**, not a provenance one: it flags that a DS-stored artifact's purpose is to be a DL entry point, independent of who authored it. A **DL-creation skill applies it to the output it authors** (a saved synthesis the user marks as an entry point at save carries it too, <u>Strategy</u> Level 4); the separate **Catalog-registration skill** then discovers `discovery-layer`-tagged records and registers them (§3). So **creation *tags*, registration *catalogs*** — the registrar's scan picks up `discovery-layer` + `ai-generated` records automatically, while a human-authored entry point is registered only through the deliberate Level 4 opt-in.
- **Provenance/verification:** `ai-generated` (default), `human-created`, `human-verified` — the automatic-vs-hand-touched axis, orthogonal to the role tag above: it governs durability and what a skill may re-derive, not whether the artifact is DL, and **not whether its content is derived**. A person can hand-author derived material, so `human-created` does not imply original content — "derived vs. original" and "auto vs. hand-touched" are separate axes.
...
**What qualifies for registration.** Registration is **keyed by `(entry_type, subject)`** — a key a non-producer looks up, not an artifact's address (one key may resolve to several rows, §3 Keys). An output qualifies only when it is **externally addressable** (answers a stable key a *non-producer* would look up), **meant to be discovered** (not a producer's private intermediate), and **worth a stable pointer** (a durable address surviving re-derivation). The **producer designates, the registrar registers**: a DL-creation skill marks what qualifies (via the record's tags) and the Catalog-registration skill catalogs it; a saved synthesis (<u>Strategy</u> Level 4) is registered only if the user opts in, as a step separate from saving the artifact. Only a **DL record** (an entry point — a skill output, or a synthesis the user marked an entry point at save) is registrable; a plain personal one-off is not, so **the Catalog only ever indexes DL records**. The rule: *register a reusable answer, keyed by a stable `(entry_type, subject)`, that consumers beyond the producer should discover.*
...
**Recovery — the registrar flags, the owner fixes.**
- **Skill-produced rows (`row_provenance = 'skill'`):** the registrar can't re-derive content, so on a drifted or transiently-unresolvable pointer it marks the row `stale`, stamps it, and surfaces it to the **producing skill** (identified by `computed_by`), which re-derives out-of-band — recompute, write to the possibly-new location, re-tag — after which the registrar picks the refreshed record up on its next scan. If the underlying source is confirmed gone, the record is no longer derivable, so the row is marked `obsolete` and stays that way until the producer drops it.
- **Human-owned rows (`row_provenance = 'human'`):** can't be recomputed either. Same stale-vs-obsolete rule applies — a drifted or transient failure marks it `stale`, a confirmed-gone or superseded source marks it `obsolete` — but instead of a producer re-deriving, the registrar surfaces it to its owner; recovery is the same revert-based path as any human-authored output. A human row is never silently deleted.
...
- **DL population** — DL-creation skills compute outputs, write each to its store via MCP, tag them, and re-derive when their sources drift. **Cataloging** — the Catalog-registration skill discovers those tagged records, registers their locations, validates that pointers resolve, and stamps freshness; it flags stale rows and surfaces them to the producer rather than re-deriving them itself.
...
- **Persisted synthesis** → a user-saved `human-created` artifact, stored under the **user's own SSO**; durable and not recomputable. It **acts as a DL record when the user marks it an entry point at save** (carrying the `discovery-layer` role tag) and as a **plain DS record otherwise** — purpose decides at save, not authorship and not registration (registration only adds one-lookup discovery; <u>Strategy</u>, Level 4). Written under the user's SSO, so it is not a service-account "DL write" (below).
...
- **Search / find** — turn a request into candidate records. This is what a <u>Strategy</u> §1.3 Query skill calls to locate the right DS records, what a §2 DL-creation skill calls to gather inputs, and what the **Catalog-registration skill** calls to discover `discovery-layer`-tagged records to register. Keyed or text search is the floor; richer matching is optional per store, and finding by tag must be supported so the registrar can locate DL records.
