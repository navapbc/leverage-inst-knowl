...
- **Skill-produced rows (`row_provenance = 'skill'`):** the registrar can't re-derive content, so on a vanished or drifted pointer it marks the row `stale`/`obsolete`, stamps it, and surfaces it to the **producing skill** (identified by `computed_by`), which re-derives out-of-band — recompute, write to the possibly-new location, re-tag — after which the registrar picks the refreshed record up on its next scan. If the underlying source is itself gone (not merely moved), the record is no longer derivable and the row stays `obsolete` until the producer drops it.
- **Human-owned rows (`row_provenance = 'human'`):** can't be recomputed either. The registrar flags the row `obsolete` and surfaces it to its owner; recovery is the same revert-based path as any human-authored output. A human row is never silently deleted.
