...
- **Role:** `discovery-layer` — a **role marker**, not a provenance one: it flags that a DS-stored artifact's purpose is to be a DL entry point, independent of who authored it. A **DL-creation skill applies it to the output it authors**, and a saved synthesis the user marks as an entry point at save (<u>Strategy</u> Level 4) carries it too — independent of whether it's registered. **Auto-registration keys on the role tag *plus* skill ownership** (`discovery-layer` + `ai-generated`), so a skill registers its own outputs automatically while a human-authored entry point is registered only through the deliberate Level 4 opt-in.
