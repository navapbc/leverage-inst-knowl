...
### 2.4 DL output types
Distilled prose, digests, indexes, prioritized pointers, retrieval hints, and content-freshness/obsolescence signals → written into a DS (such as a Confluence page), tagged `discovery-layer` (plus the key, audience, and source metadata a registrar reads to catalog it), updated in place so each re-derivation revises the same page. Freshness and pointer metadata also surface as Catalog columns (<u>Architecture</u> §3). The DL-creation skill *tags* the record; a separate Catalog-registration skill *catalogs* it (Level 3-Catalog).
...
**Who populates it.** Registration is a **separate job from creation**, often a separate team's. A **Catalog-registration skill** (the registrar) **discovers** the DL records the producers have written — scanning the sources for `discovery-layer`-tagged artifacts — and **registers** each in the Catalog, reading the record's own tags for its key, audience, and sources. The Level 2 DL-creation skills produce and tag records; they don't catalog them. This split also assigns upkeep cleanly: the registrar owns each **row** (validating that its pointer still resolves, stamping freshness, marking stale rows `obsolete`), while re-deriving stale **content** stays with the producing skill that owns it. So a broken or drifted pointer is *flagged and surfaced* by the registrar, then re-derived out-of-band by the producer — not self-healed inside one skill. (Because the registrar owns every row's upkeep, no separate reconciliation pass is needed; <u>Architecture</u> §3.)
