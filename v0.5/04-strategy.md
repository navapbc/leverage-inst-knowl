...
- **Level 3-Catalog.** One lookup to discover where any DL resource lives.
- **Level 4 — Flywheel.** Saved answers become new discoverable records — a plain DS record, or a DL record when meant as a reusable entry point — so coverage grows from the questions people actually ask.
...
**Expect many Query skills**, each **owned and versioned** so drift has a responsible fixer. **Ownership rule:** a skill exists only with a named owner (the team that owns the topic for Query skills, the source-owning team for DL-creation skills, the DL-catalog team for the Catalog-registration skill) and a demonstrated gap it closes. No owner, no skill — orphaned skills are retired, not left to rot.
...
**Goal:** stop re-searching from scratch by precomputing reusable **DL resources** — automating and scaling what the §1.3 skills did by hand.

DL resources are produced by a **DL-creation skill** under its **own non-user service identity**. DL inputs are untrusted: the skill treats DS content as data, not instructions, and an output's sharing group always comes from the skill's instructions, never inferred from content. What makes an output DL is its **purpose — a reusable entry point — not how it was produced or where it sits**; the line is **role, not location or content**. Skill-produced outputs are derived material, but a person may later edit one (§2.2), and that doesn't strip its DL role.

### 2.1 DL-creation skills — automate *what* to precompute
**Expect many DL-creation skills**, each customized to the source it handles and emitting a specific output type to a specific store, on a schedule or on demand, querying **one or more DSs** and linking across them. Each runs under a least-privilege per-DS service principal with keyless, rotated, audit-logged credentials — the same credential hygiene as the governed writer, though a **separate** identity per DS, not that single writer (mechanics: <u>Access Control</u>, <u>Storage</u>). Scheduled re-derivation also bounds staleness: a run that finds a source deleted or access revoked drops or re-restricts the derived output. *(Distinct from the §1.3 Query skills — these are the automated producers.)*

### 2.2 Authorship and durability
This is the **automatic-vs-hand-touched split within DL records.** An output is born `ai-generated` and **recomputable** from the DSs (automatic, disposable) — the default. A reviewer can promote it to `human-verified`, or a person can author one directly (`human-created`); both are **durable** — because they live in a DS, that DS backs them up, and revert is its recovery; a skill may propose a refresh but never silently overwrites them.
...
### 2.4 DL resource types
Distilled prose, digests, indexes, prioritized pointers, retrieval hints, and content-freshness/obsolescence signals → written into a DS (such as a Confluence page), carrying the DL-record marker (canonically `discovery-layer`; <u>Architecture</u> §2) plus the key, audience, and source metadata a registrar reads to catalog it, updated in place so each re-derivation revises the same page. Freshness and pointer metadata also surface as Catalog columns (<u>Architecture</u> §3). The DL-creation skill *tags* the record; a separate Catalog-registration skill *catalogs* it (Level 3-Catalog).
...
**Goal:** give every consumer **one lookup** to discover where any DL resource lives, decoupled from storage decisions.

The Catalog is DL's "yellow pages" (`entry_type + subject → location`), indexing DL's **topology**, not DS content — so a subject's pointers can migrate between stores by changing one row, with no agent change. **Keyed lookup is the guaranteed floor**: every implementation supports exact `(entry_type, subject)` retrieval, so a simple deployment needs nothing more and no consumer must depend on semantic search. Translating a fuzzy question into the right key is, by default, the **Query skill's** job. But an implementation **may** go further — a vector-DB Catalog can embed the DL records' own text and expose **semantic matching** natively, letting a consumer hand it a fuzzy question directly and shifting that fuzzy→key work from *required of the skill* to *optional*. Semantic search is thus an **opt-in capability layered on the keyed floor, never a replacement for it** (guardrails: <u>Architecture</u> §3, <u>Storage</u>). It lives at a **well-known address** agents know a priori. This mirrors Google's emerging [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md), applied to the whole DL. Design, schema, what-qualifies-for-registration, and dangling-pointer resilience are all in <u>Architecture</u> §3; promotion mechanics in <u>Storage</u>.
...
**Who populates it.** Registration is a **separate job from creation**. A **Catalog-registration skill** (the registrar) **discovers** the DL records producers have written — scanning the sources for records carrying the DL-record marker (canonically `discovery-layer`, or a declared per-source equivalent it's configured to find; <u>Architecture</u> §2) — and **registers** each, reading the record's own tags for its key, audience, and sources; the Level 2 DL-creation skills produce and tag records, they don't catalog them. Upkeep splits the same way: the registrar owns each **row** (pointer validation, freshness), the producer owns the **content** and re-derives it. So a broken or drifted pointer is *flagged and surfaced* by the registrar, then re-derived out-of-band by the producer — never self-healed in one skill (mechanics: <u>Architecture</u> §3).

**Two non-exclusive paths populate the Catalog.** The **skill path** above is one; the other is **manual (human) registration** — a person designating an existing artifact as an entry point and registering it directly (Level 4). The skill path enumerates *marked* records, so it depends on the marker; the manual path has no discovery scan, so the marker is optional and the row itself carries the entry-point role. Manual rows are **human-owned** — no skill re-derives them.
...
## Level 4 — Flywheel: saved answers become new discoverable records

**Goal:** grow DL coverage from the questions people actually ask. When a Query skill synthesizes an answer (from many records, in one DS or several) that exists in no DL resource yet, it offers to persist that synthesis as a new durable record — so the next person retrieves it instead of re-deriving it.

The gap this closes: Level 2's skills must **guess** what to precompute, and Level 3-Confirmations only captures trust on outputs that *already exist*. Level 4 makes every saved novel synthesis a candidate for one-lookup discovery — a synthesis the user marks as an entry point acts as a DL record, and registering it adds one-lookup discovery — so coverage grows **demand-driven**. Builds on Level 1 (the §1.3 Query skills and §1.2 write model). Level 3-Catalog is **optional but strongly recommended**: without it a saved synthesis still exists and is reachable by skill routing or bounded fan-out, but the Catalog is what makes it discoverable in one lookup. Does *not* depend on Level 3-Confirmations.

### 4.1 The save-to-create gesture
When the skill synthesizes an answer matching no existing DL resource, it offers to **create** it — *"Create a Confluence page / Google Doc from this answer?"* The skill knows whether it answered from an existing output or had to assemble one from scratch. The user can save the **whole synthesis or just the good section**. **Opt-in and user-driven** — the user vouches for both the content and the action; no silent writes.

### 4.2 What gets written, and how
A **`human-created`** record — durable, not recomputable, attributed in version history — written under the **user's own SSO** into a DS-hosted store (a Confluence page or Google Doc). Whether it **acts as a DL record or a plain DS record turns on purpose, decided at save, not on who authored it or whether it's later registered**: if the user marks it as a reusable discovery entry point, it acts as a **`human-created` DL record**, carrying the role-marking `discovery-layer` tag; if it's a personal one-off answer, it stays a **plain DS record**. Registration (below) is a *separate* choice about discoverability. Either way it's durable and no skill re-derives it — the `discovery-layer` tag here flags *role*, and auto-re-derivation keys on skill-owned (`ai-generated`) outputs, not this one (<u>Architecture</u> §2–3). Because a synthesis can draw on records with different restrictions, it can **blend tiers**, so it isn't covered by §1.2's single-location inheritance. **The saving user sets the record's audience** when they save it, under their own SSO — and is **responsible for choosing one no broader than the sources it drew on** (default-deny if they specify nothing). The skill may surface the sources' restrictions to inform that choice, but access is the user's call, not the skill's. (Access model: <u>Architecture</u> §4; <u>Access Control</u>.)

**Cataloging is a second, separate opt-in.** Saving the record does *not* register it. A saved synthesis exists and is reachable by skill routing or fan-out on its own; promoting it to **one-lookup discovery** is a distinct choice the user makes only when the answer earns a stable pointer — it answers a durable `(entry_type, subject)` key a non-producer would look up, not a one-off personal Q&A (the §3 qualification rules, <u>Architecture</u>). Registration changes only *how discoverable* the record is — one lookup vs. skill routing / fan-out — not whether it's a DL record (that was set at save, above). The two opt-ins are separate but **not independent**: registration is offered only for a record marked an entry point (a DL record), never a personal one-off — so the Catalog only ever indexes DL records. When the user opts to register, the governed writer writes a Catalog pointer with the user attributed in `updated_by` and `row_provenance = 'human'` (the same narrow-writer split confirmations use) — a **human-owned row** no skill re-derives, distinct from the record's own `discovery-layer` role tag. Default off, so the Catalog stays small and stable rather than accumulating every saved answer. (Write split: <u>Architecture</u> §4; what qualifies for registration: <u>Architecture</u> §3.)

**This manual (human) registration path is not limited to an in-session synthesis.** A person may also **designate a pre-existing artifact** — a page, doc, or sheet they already know is the best entry point for a topic — and register it through the same path. What differs from the save-to-create gesture is only *origin*: no new artifact is authored, so there is nothing to write to a DS and **no need for write access to the target** (tagging the source `discovery-layer` is optional; the row carries the role). **The registrant also need not be the record's author** — they are designating an entry point they know is best for a topic, not claiming authorship of it. The person points the agent at the existing record and it becomes a DL record by being registered as an entry point. Everything else matches the saved-synthesis case — a **human-owned row** (`row_provenance = 'human'`) no skill re-derives, an audience no broader than the source, and `verification` set from whether the person is **vouching** for the record or merely **flagging** it a useful pointer (which also sets its lookup ranking, <u>Architecture</u> §2–3). This is the manual counterpart to the registrar's automated skill path (Level 3-Catalog); the two are non-exclusive.
...
3. The synthesis is now a durable record — a DL record if the user marked it an entry point, otherwise a plain DS record. If also registered, it's discoverable in one lookup; if not, it's still reachable by skill routing or fan-out — just not in one step.
