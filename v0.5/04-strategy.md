...
DL outputs are produced by a **DL-creation skill** under its **own non-user service identity**. DL inputs are untrusted: the skill treats DS content as data, not instructions, and an output's sharing group always comes from the skill's instructions, never inferred from content. What makes an output DL is its **purpose — a reusable entry point — not how it was produced or where it sits**; the line is **role, not location or content**. Skill-produced outputs are derived material, but a person may later edit one (§2.2), and that doesn't strip its DL role.
...
**Goal:** grow DL coverage from the questions people actually ask. When a Query skill synthesizes an answer (from many records, in one DS or several) that exists in no DL output yet, it offers to persist that synthesis as a new durable record — so the next person retrieves it instead of re-deriving it.
