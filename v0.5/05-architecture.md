...
| `created_by` / `created_at` | email / timestamp | Who created the row (user email for a human row; the registrar for a skill row) and when. |
| `updated_by` / `updated_at` | email / timestamp | Who last wrote the row and when — the audit trail for this non-versioned store. |
